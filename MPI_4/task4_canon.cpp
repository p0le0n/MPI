...
// Linear matrix multiplication for comparison (only for the root process)
void linearMatrixMultiply(int **a, int **b, int rows, int cols, int **c) {
    for (int i = 0; i < rows; i++) {
        for (int j = 0; j < cols; j++) {
            c[i][j] = 0;
            for (int k = 0; k < rows; k++) {
...
int main(int argc, char* argv[]) {
    ...
    // Time measurement for MPI-based multiplication
    double mpiEndTime = MPI_Wtime();
    if (rank == 0) {
        printf("MPI Matrix multiplication time: %.5f seconds\n", mpiEndTime - startTime);
    }

    // Linear Matrix Multiplication (only on rank 0 for comparison)
    if (rank == 0) {
        double startTime = MPI_Wtime();
        int **linearC = NULL;
        if (allocMatrix(&linearC, rows, columns) != 0) {
            printf("[ERROR] Matrix alloc for linearC failed!\n");
            MPI_Abort(MPI_COMM_WORLD, 6);
        }
        linearMatrixMultiply(A, B, rows, columns, linearC);
        double endTime = MPI_Wtime();
        printf("Linear Matrix multiplication time: %.5f seconds\n", endTime - startTime);
        freeMatrix(&linearC);
    }
